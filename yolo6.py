...
from nets.vgg_classifier import V_c_net
...
class YOLO(object):
    ...
    def generate(self):
        ...
        # pretrained_dict = torch.load('model_data/yolo4_new_weights.ph', map_location=device)
        # pretrained_dict = torch.load('logs/Epoch1-Total_Loss407.6023-Val_Loss39.2069.pth', map_location=device)
        # pretrained_dict = torch.load('model_data/yolo4_voc_weights.pth', map_location=device)
...
###########################
        self.classnet = V_c_net("glo_classifier", pretraining=True, num_class=5)#uncertain 后期判断
        #
        ...
        pretrained_dict2 = {k: v for k, v in pretrained_dict2.items() if
                           (k in model_dict2) and (np.shape(model_dict2[k]) == np.shape(v))}
        model_dict2.update(pretrained_dict2)
        self.classnet.load_state_dict(model_dict2)
        print('class model load Finished!')
##########################
        ...
        if self.cuda:
            ...
            self.classnet = nn.DataParallel(self.classnet)
            self.classnet = self.classnet.cuda()
        print('Finished!')
    ...
    def detect_image(self, image):
        ...
        font = ImageFont.truetype(font='model_data/simhei.ttf', size=np.floor(2.5e-2 * np.shape(image)[0] + 0.5).astype('int32'))
