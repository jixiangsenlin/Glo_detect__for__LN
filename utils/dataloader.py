...
class YoloDataset(Dataset):
    def __init__(self, train_lines, image_size, mosaic=True):
        ...
        self.image_size = image_size#768 256      256 768
        self.mosaic = mosaic
        self.flag = True
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=1.5, val=1.5):
        ...
        # iw, ih = image.size
        h, w = input_shape# 768 256   反了吧 应该是    w = 768   h = 256
    ...
    def __getitem__(self, index):
        ...
        if len(y) != 0:
            ...
            boxes[:, 0] = boxes[:, 0] / self.image_size[1]# xmin w
            boxes[:, 1] = boxes[:, 1] / self.image_size[0]# ymin h
            boxes[:, 2] = boxes[:, 2] / self.image_size[1]
            boxes[:, 3] = boxes[:, 3] / self.image_size[0]

            boxes = np.maximum(np.minimum(boxes, 1), 0)
            boxes[:, 2] = boxes[:, 2] - boxes[:, 0]# object_w
            boxes[:, 3] = boxes[:, 3] - boxes[:, 1]# object_h

            boxes[:, 0] = boxes[:, 0] + boxes[:, 2] / 2# object_center_x
            boxes[:, 1] = boxes[:, 1] + boxes[:, 3] / 2# object_center_y
