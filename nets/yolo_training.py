...
class YOLOLoss(nn.Module):
    def __init__(self, anchors, num_classes, img_size, label_smooth=0, cuda=True):
        ...
        self.feature_length = [img_size[0]//32,img_size[0]//16,img_size[0]//8]#h 256   768
        # self.feature_lengthw = [img_size[1] // 32, img_size[1] // 16, img_size[1] // 8]#w 768    256
    ...
    def forward(self, input, targets=None):
        ...
        in_h = input.size(2)#8
        # 特征层的宽
        in_w = input.size(3)#24

        # 计算步长
        # 每一个特征点对应原来的图片上多少个像素点
        # 如果特征层为13x13的话，一个特征点就对应原来的图片上的32个像素点
        stride_h = self.img_size[1] / in_h # 256/8=32
        stride_w = self.img_size[0] / in_w # 768/24=32

        # 把先验框的尺寸调整成特征层大小的形式
        # 计算出先验框在特征层上对应的宽高
        scaled_anchors = [(a_w / stride_w, a_h / stride_h) for a_w, a_h in self.anchors]
        # scaled_anchors = [(a_w / stride_w * 1000/256, a_h / stride_h * 1000/256) for a_w, a_h in self.anchors]
        # bs,3*(5+num_classes),13,13 -> bs,3,13,13,(5+num_classes)
        prediction = input.view(bs, int(self.num_anchors/3),
                                self.bbox_attrs, in_h, in_w).permute(0, 1, 3, 4, 2).contiguous()
        
        # 对prediction预测进行调整
        conf = torch.sigmoid(prediction[..., 4])  # Conf 置信度 confidence
        pred_cls = torch.sigmoid(prediction[..., 5:])  # Cls pred. 概率 probability
